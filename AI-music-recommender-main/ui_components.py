...
Import and call these functions from app.py.
"""
...
# ─────────────────────────────────────────────────────────────────────
# 1. GLOBAL CSS — glassmorphism, animations, mobile, all card classes
# ─────────────────────────────────────────────────────────────────────
def inject_global_css(accent: str = "#1DB954", bg: str = ""):
    if not bg:
    ...
    .song-card {{
        ...
        transition: transform 0.25s cubic-bezier(.4,2,.6,1),
                    box-shadow 0.25s ease, border-color 0.3s ease;
        overflow: hidden;
    }}
    .song-card::before {{
        content: '';
    ...
    .song-card h2 {{ margin: 0 0 6px 0; font-size: 1.35rem; font-weight: 800; letter-spacing: -0.3px; }}
    .song-card .meta {{ opacity: 0.7; font-size: 0.88rem; margin-bottom: 14px; }}
    .song-card .pill-btn {{
        display: inline-block; margin: 4px 6px 4px 0;
        padding: 7px 18px; border-radius: 50px; font-weight: 700;
        font-size: 0.85rem; text-decoration: none; transition: transform 0.15s, opacity 0.15s;
    }}
    .song-card .pill-btn:hover {{ transform: scale(1.06); opacity: 0.9; }}
    /* Legacy .song-box kept for compatibility */
    .song-box {{ background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1);
        backdrop-filter:blur(12px); padding:25px; border-radius:16px;
        border-left:6px solid {accent}; color:white; text-align:center;
    ...
    @keyframes floatNote {{
        ...
        20%  {{ opacity:0.7; }}
        80%  {{ opacity:0.4; }}
        100% {{ opacity:0; transform: translateY(-70px) rotate(15deg); }}
    }}
    /* ── Mood selector cards ── */
    .mood-grid {{ display:flex; gap:14px; justify-content:center; flex-wrap:wrap; margin: 10px 0 18px 0; }}
    .mood-card {{
        cursor: pointer;
        background: rgba(255,255,255,0.06);
    ...
    .mood-card:hover {{ transform: scale(1.08); border-color: {accent}; background: rgba(255,255,255,0.1); }}
    .mood-card.active {{ border-color: {accent}; background: {accent}22;
        box-shadow: 0 0 20px {accent}44; transform: scale(1.06); }}
    .mood-card .mood-emoji {{ font-size: 2.4rem; display:block; margin-bottom:6px; }}
    .mood-card .mood-label {{ font-size: 0.88rem; font-weight: 700; opacity: 0.9; }}
    /* ── Skeleton loading ── */
    .skeleton {{
        background: linear-gradient(90deg, rgba(255,255,255,0.06) 25%,
                    rgba(255,255,255,0.13) 50%, rgba(255,255,255,0.06) 75%);
    ...
    .skeleton-line {{ height: 14px; border-radius: 7px; margin: 10px 0; }}
    .skeleton-title {{ height: 22px; width: 60%; border-radius: 8px; margin-bottom: 14px; }}
    ...
    .plm-toast {{
        ...
        box-shadow: 0 8px 32px rgba(0,0,0,0.5), 0 0 20px {accent}22;
        display: flex; align-items: center; gap: 12px;
        animation: toastIn 0.35s cubic-bezier(.4,2,.6,1) forwards,
                   toastOut 0.35s ease 3.5s forwards;
    ...
    .now-playing-bar .np-title {{ font-weight: 700; font-size: 0.92rem;
        white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
    .now-playing-bar .np-meta  {{ font-size: 0.78rem; opacity: 0.6; margin-top: 1px; }}
    .now-playing-bar .np-eq    {{
        display: flex; align-items: flex-end; gap: 3px; height: 22px; flex-shrink: 0;
    }}
    .np-eq span {{
        display: inline-block; width: 4px; background: {accent};
        border-radius: 2px; animation: eqBounce 0.8s ease-in-out infinite;
    }}
    .np-eq span:nth-child(2) {{ animation-delay: 0.15s; }}
    .np-eq span:nth-child(3) {{ animation-delay: 0.3s; }}
    .np-eq span:nth-child(4) {{ animation-delay: 0.1s; }}
    @keyframes eqBounce {{
        0%,100% {{ height: 4px; }}
        50%      {{ height: 18px; }}
    }}
    ...
    .sidebar-avatar {{
        ...
        font-size: 1.8rem; margin: 0 auto 10px auto;
        box-shadow: 0 0 20px {accent}55;
    }}
    .sidebar-username {{ text-align:center; font-weight:800; font-size:1.1rem; margin-bottom:2px; }}
    .sidebar-level    {{ text-align:center; font-size:0.8rem; opacity:0.6; margin-bottom:12px; }}
    .sidebar-stat-row {{
    ...
    .sidebar-stat .val {{ font-weight:800; font-size:1.1rem; color:{accent}; }}
    .sidebar-stat .lbl {{ font-size:0.72rem; opacity:0.55; }}
    .queue-item {{
        display: flex; align-items: center; gap: 8px;
        background: rgba(255,255,255,0.04); border-radius: 8px;
        padding: 6px 10px; margin-bottom: 5px; font-size: 0.85rem;
    ...
    .onboard-step-dots {{ display:flex; justify-content:center; gap:8px; margin:24px 0 8px 0; }}
    .onboard-dot {{
        width:10px; height:10px; border-radius:50%;
        background: rgba(255,255,255,0.2); transition: background 0.3s;
    }}
    .onboard-dot.active {{ background: {accent}; box-shadow: 0 0 8px {accent}; }}
    /* ── History / misc cards ── */
    .history-card {{
    ...
    .stTabs [data-baseweb="tab"] {{
        height: 42px; background-color: rgba(255,255,255,0.04) !important;
        border-radius: 10px 10px 0 0; padding: 6px 14px; font-size: 0.92rem;
        transition: background 0.2s;
    }}
    .stTabs [data-baseweb="tab"]:hover {{ background-color: rgba(255,255,255,0.09) !important; }}
    .stTabs [aria-selected="true"] {{ background-color: {accent}22 !important;
    ...
    @media (max-width: 768px) {{
        .song-card {{ padding: 16px 14px; border-radius: 14px; }}
        .song-card h2 {{ font-size: 1.1rem; }}
        .plm-header h1 {{ font-size: 1.8rem; }}
        .mood-card {{ min-width: 80px; padding: 12px 14px; }}
        .mood-card .mood-emoji {{ font-size: 1.8rem; }}
        .dna-card {{ padding: 18px 14px; }}
        .badge-card {{ min-width: 90px; padding: 8px 10px; }}
        .onboard-wrap {{ padding: 24px 18px; margin: 16px; }}
...
# 2. ANIMATED HEADER
# ─────────────────────────────────────────────────────────────────────
def render_animated_header(mood: str = "Relaxed"):
    ...
    for i in range(8):
        ...
        notes_html += (
            f"<span class='note' style='left:{left}%;animation-delay:{delay}s;'>{note}</span>"
        )
    st.markdown(f"""
    <div class='plm-header'>
        <div class='music-notes'>{notes_html}</div>
        <h1>🎵 PLM Devs Music AI</h1>
        <p style='opacity:0.6;font-size:1rem;margin:6px 0 0 0;letter-spacing:0.5px;'>
...
# ─────────────────────────────────────────────────────────────────────
# 3. ANIMATED GLASSMORPHISM SONG CARD
# ─────────────────────────────────────────────────────────────────────
def render_song_card(song: dict, mood: str = "Relaxed"):
    """Render a single glassmorphism song card (HTML only, no buttons)."""
    ...
    energy_badge = {
        ...
        "Medium": f"<span style='background:#ffa50022;color:#ffcc66;border:1px solid #ffa50044;border-radius:20px;padding:2px 10px;font-size:0.78rem;'>⚡ Medium</span>",
    ...
    <div class='song-card' style='border-left-color:{accent};'>
        ...
        <div class='meta'>
            🎭 {song.get('Mood','')} &nbsp;·&nbsp;
            🎸 {song.get('Genre','')} &nbsp;·&nbsp;
            🌐 {song.get('Language','')} &nbsp; {energy_badge}
        </div>
...
# ─────────────────────────────────────────────────────────────────────
# 4. MOOD SELECTOR (emoji cards, returns selected mood string)
# ─────────────────────────────────────────────────────────────────────
def render_mood_selector(current_mood: str = "Relaxed") -> str:
    """
    Renders 4 clickable mood cards using Streamlit columns + HTML styling.
    Returns the mood string that was clicked (or current_mood if none clicked).
    """
    ...
    for col, (mood_name, emoji, color, subtitle) in zip(cols, moods):
        is_active = current_mood == mood_name
        ...
        with col:
            ...
            <div style='background:{bg};border:{border};border-radius:18px;
                ...
                <div style='font-weight:800;font-size:0.92rem;margin-top:6px;'>{mood_name}</div>
                <div style='font-size:0.72rem;opacity:0.6;margin-top:2px;'>{subtitle}</div>
            </div>
            """, unsafe_allow_html=True)
            if st.button(f"Select {mood_name}", key=f"moodsel_{mood_name}",
...
def render_skeleton_cards(n: int = 3):
    """Show n skeleton placeholder cards while recommendations load."""
    for _ in range(n):
        st.markdown("""
        <div class='skeleton-card skeleton'>
            <div class='skeleton-title skeleton'></div>
            <div class='skeleton-line skeleton' style='width:45%;'></div>
            <div class='skeleton-line skeleton' style='width:70%;margin-top:14px;'></div>
...
def show_toast(message: str, icon: str = "🎵", mood: str = "Relaxed"):
    """Render a custom styled toast notification (top-right)."""
    accent = MOOD_ACCENT.get(mood, "#1DB954")
    st.markdown(f"""
    <div class='plm-toast' style='border-left-color:{accent};box-shadow:0 8px 32px rgba(0,0,0,0.5),0 0 20px {accent}22;'>
...
def render_now_playing_bar(song_name: str, mood: str, genre: str,
    ...
    queue_badge = (
        f"<span style='background:rgba(255,255,255,0.1);border-radius:20px;"
        f"padding:3px 10px;font-size:0.78rem;'>🗂️ {queue_count} in queue</span>"
    ...
    <div class='now-playing-bar'>
        ...
        <div class='np-info'>
            <div class='np-title'>🎵 {song_name}</div>
            <div class='np-meta'>🎭 {mood} &nbsp;·&nbsp; 🎸 {genre} &nbsp; {queue_badge}</div>
        </div>
        <div style='display:flex;gap:8px;flex-shrink:0;'>
            <a href='{sp_url}' target='_blank'
               style='background:#1DB954;color:#000;border-radius:20px;padding:5px 14px;
               font-size:0.8rem;font-weight:700;text-decoration:none;white-space:nowrap;'>
               🎧 Spotify</a>
            <a href='{yt_url}' target='_blank'
               style='background:#FF0000;color:#fff;border-radius:20px;padding:5px 14px;
               font-size:0.8rem;font-weight:700;text-decoration:none;white-space:nowrap;'>
               ▶ YouTube</a>
        </div>
    </div>
...
def render_sidebar(username: str, xp: int, level: str, streak: int,
    ...
    st.markdown(f"""
    <div class='sidebar-avatar' style='background:linear-gradient(135deg,{accent},{accent}88);'>
        {initials}
    </div>
    <div class='sidebar-username'>{username}</div>
    <div class='sidebar-level'>{level}</div>
    """, unsafe_allow_html=True)
    ...
    pct = int((xp - prev_thresh) / max(next_thresh - prev_thresh, 1) * 100)
    st.markdown(f"""
    <div style='margin-bottom:4px;display:flex;justify-content:space-between;font-size:0.75rem;opacity:0.6;'>
        <span>⚡ {xp} XP</span><span>{next_thresh - xp} to next</span>
    </div>
    <div style='background:rgba(255,255,255,0.1);border-radius:20px;height:6px;margin-bottom:14px;'>
        <div style='background:{accent};width:{pct}%;height:6px;border-radius:20px;
            box-shadow:0 0 8px {accent}88;transition:width 0.5s;'></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class='sidebar-stat-row'>
        <div class='sidebar-stat'><div class='val'>🔥{streak}</div><div class='lbl'>Streak</div></div>
        <div class='sidebar-stat'><div class='val' style='color:{accent};'>{xp}</div><div class='lbl'>XP</div></div>
        <div class='sidebar-stat'><div class='val'>❤️{len(favs)}</div><div class='lbl'>Favs</div></div>
    </div>
    """, unsafe_allow_html=True)

    # Queue section
    st.markdown(f"<p style='font-weight:700;font-size:0.85rem;opacity:0.7;margin:0 0 6px 0;'>"
                f"🗂️ Queue ({len(queue)})</p>", unsafe_allow_html=True)
    if not queue:
        st.markdown("<p style='font-size:0.8rem;opacity:0.4;font-style:italic;'>Empty — like a song to add!</p>",
                    unsafe_allow_html=True)
    else:
        for item in queue[::-1][:6]:
            st.markdown(f"<div class='queue-item'>🎵 {item}</div>", unsafe_allow_html=True)
        if len(queue) > 6:
            st.markdown(f"<p style='font-size:0.75rem;opacity:0.4;text-align:center;'>+{len(queue)-6} more</p>",
                        unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────────────────
# 9. ONBOARDING SCREEN (3-step wizard for first-time users)
# ─────────────────────────────────────────────────────────────────────
def render_onboarding() -> bool:
    """
    Shows a 3-step onboarding wizard. Returns True when onboarding is complete.
    Reads/writes st.session_state.onboard_step.
    """
    ...
    # Step dots
    dots_html = "<div class='onboard-step-dots'>"
    for i in range(1, 4):
        cls = "onboard-dot active" if i == step else "onboard-dot"
        dots_html += f"<div class='{cls}'></div>"
    dots_html += "</div>"

    STEPS = {
        1: ("👋", "Welcome to PLM Music AI!",
            "Ek personal AI jo tere saath music taste seekhta hai — tera mood, teri language, tera vibe.",
            ["🧠 Q-Learning Engine", "🎭 Mood Detection", "🏆 Gamification"]),
        2: ("🎭", "Apna Pehla Mood Batao",
            "Abhi is waqt kaisa feel ho raha hai? AI isi se start karega.",
            None),
        3: ("🌍", "Language Choose Karo",
            "Kaunsi language mein gaane sunna chahte ho?",
            None),
    }
        ...
        if step == 1:
            if features:
                for feat in features:
                    st.markdown(
                        f"<div style='background:rgba(29,185,84,0.1);border:1px solid #1DB95444;"
                        f"border-radius:10px;padding:10px 16px;margin:6px 0;font-weight:600;'>"
                        f"{feat}</div>", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            for label in langs:
                if st.button(label, use_container_width=True, key=f"ob_lang_{label}"):
                    st.session_state.current_language = lang_map[label]
                    st.session_state.onboard_done = True
                    st.session_state.onboard_step = 1  # reset for future logins
                    st.rerun()
