    ...
    with st.expander("⏱️ Pomodoro & Sleep Timer",
        ...
        with pomo_tab:
            ...
            with c1:
                ...
                preset_map = {"25 min Focus": 25, "50 min Deep Work": 50,
                              "5 min Break": 5, "10 min Break": 10}
                mins = (st.number_input("Minutes", 1, 120, 25,
                                        disabled=st.session_state.pomo_active, key="pomo_mins")
                        if preset == "Custom" else preset_map[preset])
            with c2:
                if not st.session_state.pomo_active:
                    if st.button("▶ Start", use_container_width=True,
                                 type="primary", key="pomo_start"):
            ...
            if st.session_state.pomo_active:
                ...
                if remaining <= 0:
                    ...
                    st.markdown(
                        f"<h2 style='text-align:center;color:#1DB954;font-size:3rem;'>"
                        f"⏳ {m:02d}:{s:02d}</h2>", unsafe_allow_html=True)
        ...
        with sleep_tab:
            ...
            if st.session_state.sleep_active:
                ...
                if remaining <= 0:
                    ...
                    st.markdown(
                        f"<p style='text-align:center;color:#aaa;font-size:1.5rem;'>"
                        f"😴 Stopping in {m:02d}:{s:02d}</p>", unsafe_allow_html=True)
                    time.sleep(1)
                    st.rerun()
