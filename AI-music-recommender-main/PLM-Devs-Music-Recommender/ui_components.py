...
def render_animated_header(mood: str = "Relaxed"):
    ...
    notes = ["♪", "♫", "♬", "🎵", "🎶"]
    notes_html = "".join(
        f"<span style='position:absolute;font-size:1.5rem;opacity:0;left:{random.randint(5,95)}%;"
        f"animation:floatNote 4s ease-in-out {round(random.uniform(0,4),1)}s infinite;'>{notes[i%5]}</span>"
        for i in range(8)
    )
    st.markdown(f"""
    <style>
    ...
    .plm-header h1 {{ font-size:2.6rem; font-weight:800; letter-spacing:-1px;
        background:linear-gradient(90deg,{accent},#ffffff,{accent}); background-size:200% auto;
        -webkit-background-clip:text; -webkit-text-fill-color:transparent;
        animation:shimmer 3s linear infinite; margin:0; }}
    </style>
    <div class='plm-header'>
        <div style='position:absolute;top:18px;left:50%;transform:translateX(-50%);width:100%;overflow:hidden;height:80px;pointer-events:none;'>
            {notes_html}
        </div>
...
def render_song_card(song: dict, mood: str = "Relaxed"):
    ...
    .pill-btn {{ display:inline-block; margin:4px 6px 4px 0; padding:7px 18px; border-radius:50px;
        font-weight:700; font-size:0.85rem; text-decoration:none; transition:transform 0.15s; }}
    .pill-btn:hover {{ transform:scale(1.06); }}
    </style>
    <div class='song-card'>
        <h2>{song['Song']}</h2>
        <div class='meta'>🎭 {song.get('Mood','')} &nbsp;·&nbsp; 🎸 {song.get('Genre','')} &nbsp;·&nbsp; 🌐 {song.get('Language','')} &nbsp; {energy_badge}</div>
        <a class='pill-btn' href='{sp_url}' target='_blank' style='background:#1DB954;color:#000;'>🎧 Spotify</a>
        <a class='pill-btn' href='{yt_url}' target='_blank' style='background:#FF0000;color:#fff;'>▶ YouTube</a>
    </div>""", unsafe_allow_html=True)
...
def render_mood_selector(current_mood: str = "Relaxed") -> str:
    ...
    for col, (mood_name, emoji, color, subtitle) in zip(cols, moods):
        ...
        with col:
            st.markdown(f"""<div style='background:{bg};border:{border};border-radius:18px;
                padding:16px 10px;text-align:center;box-shadow:{shadow};transition:all 0.2s;'>
            ...
            if st.button(f"Select {mood_name}", key=f"moodsel_{mood_name}", use_container_width=True,
...
def render_skeleton_cards(n: int = 3):
    for _ in range(n):
        st.markdown("""
        <style>
        .skeleton { background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.13) 50%,rgba(255,255,255,0.06) 75%);
            background-size:200% 100%; animation:skeletonPulse 1.4s ease infinite; border-radius:12px; }
...
def show_toast(message: str, icon: str = "🎵", mood: str = "Relaxed"):
    ...
    st.markdown(f"""
    <style>
    .plm-toast {{ position:fixed; top:20px; right:20px; z-index:99999; min-width:280px; max-width:360px;
        background:rgba(20,20,30,0.92); border:1px solid {accent}66; border-left:4px solid {accent};
        border-radius:14px; padding:14px 18px; backdrop-filter:blur(16px);
        box-shadow:0 8px 32px rgba(0,0,0,0.5); display:flex; align-items:center; gap:12px;
        animation:toastIn 0.35s cubic-bezier(.4,2,.6,1) forwards, toastOut 0.35s ease 3.5s forwards;
    ...
    <div class='plm-toast'><span style='font-size:1.4rem;'>{icon}</span><span>{message}</span></div>
    """, unsafe_allow_html=True)


def render_now_playing_bar(song_name: str, mood: str, genre: str, queue_count: int = 0, mood_color: str = "#1DB954"):
    ...
    queue_badge = f"<span style='background:rgba(255,255,255,0.1);border-radius:20px;padding:3px 10px;font-size:0.78rem;'>🗂️ {queue_count} in queue</span>" if queue_count > 0 else ""
    st.markdown(f"""
    <style>
    .now-playing-bar {{ position:fixed; bottom:0; left:0; right:0; z-index:9999;
        background:rgba(10,10,18,0.88); border-top:1px solid {mood_color}44;
        backdrop-filter:blur(20px); padding:10px 24px;
        display:flex; align-items:center; gap:16px; box-shadow:0 -4px 30px rgba(0,0,0,0.5); }}
    .np-pulse {{ width:10px; height:10px; border-radius:50%; background:{mood_color}; animation:npPulse 1.2s ease-in-out infinite; flex-shrink:0; }}
    ...
    .np-eq span {{ display:inline-block; width:4px; background:{mood_color}; border-radius:2px; animation:eqBounce 0.8s ease-in-out infinite; }}
    ...
    <div class='now-playing-bar'>
        ...
        <div style='flex:1;min-width:0;'>
            <div style='font-weight:700;font-size:0.92rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>🎵 {song_name}</div>
            <div style='font-size:0.78rem;opacity:0.6;margin-top:1px;'>🎭 {mood} &nbsp;·&nbsp; 🎸 {genre} &nbsp; {queue_badge}</div>
        </div>
        <div style='display:flex;gap:8px;flex-shrink:0;'>
            <a href='{sp_url}' target='_blank' style='background:#1DB954;color:#000;border-radius:20px;padding:5px 14px;font-size:0.8rem;font-weight:700;text-decoration:none;'>🎧 Spotify</a>
            <a href='{yt_url}' target='_blank' style='background:#FF0000;color:#fff;border-radius:20px;padding:5px 14px;font-size:0.8rem;font-weight:700;text-decoration:none;'>▶ YouTube</a>
        </div>
    </div>""", unsafe_allow_html=True)


def render_sidebar(username: str, xp: int, level: str, streak: int, queue: list, favs: list, mood: str = "Relaxed"):
    accent = MOOD_ACCENT.get(mood, "#1DB954")
    ...
    <div style='display:flex;justify-content:space-between;font-size:0.75rem;opacity:0.6;margin-bottom:4px;'><span>⚡ {xp} XP</span><span>{next_thresh-xp} to next</span></div>
    ...
    <p style='font-weight:700;font-size:0.85rem;opacity:0.7;margin:0 0 6px 0;'>🗂️ Queue ({len(queue)})</p>
    """, unsafe_allow_html=True)
    if not queue:
...
def render_onboarding() -> bool:
    ...
    dots_html = "".join(f"<div style='width:10px;height:10px;border-radius:50%;background:{'#1DB954' if i==step else 'rgba(255,255,255,0.2)'};display:inline-block;margin:0 4px;'></div>" for i in range(1,4))
    STEPS = {
        1: ("👋", "Welcome to PLM Music AI!", "Ek personal AI jo tere saath music taste seekhta hai.", ["🧠 Q-Learning Engine", "🎭 Mood Detection", "🏆 Gamification"]),
    ...
    <div class='onboard-wrap'>
        ...
        <div style='display:flex;justify-content:center;gap:8px;margin:24px 0 8px 0;'>{dots_html}</div>
    </div>""", unsafe_allow_html=True)
    ...
    with center_col:
        if step == 1:
            ...
            lang_map = {"🇮🇳 Hindi":"Hindi","🇬🇧 English":"English","🎺 Punjabi":"Punjabi","🎵 Tamil":"Tamil","🎶 Telugu":"Telugu"}
