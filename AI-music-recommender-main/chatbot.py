...
class MusicChatbot:
    ...
    def respond(self, user_input: str) -> dict:
        ...
        if 'help' in text:
            return {"text": "Main tumhare liye songs dhundh sakta hoon!\n- 'Sad Hindi songs do'\n- 'Party punjabi'\n- 'Focus ke liye lofi'\n- 'Surprise me!'", "songs": []}
        if 'surprise' in text or 'random' in text or 'kuch bhi' in text:
            return {"text": "🎲 Surprise! Yeh lo random picks:", "songs": self.df.sample(min(3, len(self.df))).to_dict('records')}
        ...
        if mood is None and lang is None and genre is None:
            return {"text": "Hmm, samajh nahi aaya 🤔 Example: 'sad hindi songs', 'party punjabi', 'lofi focus music'", "songs": []}
        ...
        if len(filtered) < 3:
            if mood: filtered = self.df[self.df['Mood'] == mood]
            elif lang: filtered = self.df[self.df['Language'].str.lower() == lang.lower()]
        if len(filtered) == 0:
            filtered = self.df

        songs = filtered.sample(min(3, len(filtered))).to_dict('records')
        parts = [x for x in [mood, lang, genre] if x]
        label = " + ".join(parts) if parts else "random"
        return {"text": random.choice([f"🎵 Yeh lo {label} songs!", f"Found some great {label} tracks 🎧", f"Here are your {label} picks ✨"]), "songs": songs}
