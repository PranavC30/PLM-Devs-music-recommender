...
def render_pomodoro():
    ...
    with st.expander("⏱️ Pomodoro & Sleep Timer",
                     expanded=st.session_state.pomo_active or st.session_state.sleep_active):
        ...
        with pomo_tab:
            ...
            with c1:
                preset = st.selectbox("Preset",
                    ["25 min Focus","50 min Deep Work","5 min Break","10 min Break","Custom"],
            ...
            with c2:
                if not st.session_state.pomo_active:
                    if st.button("▶ Start",use_container_width=True,type="primary",key="pomo_start"):
                        st.session_state.pomo_active=True; st.session_state.pomo_duration=mins*60
                        st.session_state.pomo_end_time=time.time()+mins*60; st.rerun()
                else:
                    if st.button("⏹ Stop",use_container_width=True,key="pomo_stop"):
                        st.session_state.pomo_active=False; st.rerun()
            with c3:
            ...
            if st.session_state.pomo_active:
                ...
                if remaining <= 0:
                    st.session_state.pomo_active=False; st.session_state.pomo_completed+=1
                    st.success("🎉 Session complete! Take a break."); st.balloons(); st.rerun()
                else:
                    st.progress(1-(remaining/st.session_state.pomo_duration))
                    m,s = divmod(remaining,60)
                    st.markdown(f"<h2 style='text-align:center;color:#1DB954;font-size:3rem;'>⏳ {m:02d}:{s:02d}</h2>",unsafe_allow_html=True)
                    st.info("Stay focused! 🎵"); time.sleep(1); st.rerun()
            ...
            with s2:
                if not st.session_state.sleep_active:
                    if st.button("▶ Start Sleep Timer",use_container_width=True,key="sleep_start"):
                        ...
                        st.session_state.sleep_total_duration=sleep_mins*60; st.rerun()
                else:
                    if st.button("⏹ Cancel",use_container_width=True,key="sleep_stop"):
                        st.session_state.sleep_active=False; st.rerun()
            if st.session_state.sleep_active:
                remaining = int(st.session_state.sleep_end_time - time.time())
                if remaining <= 0:
                    st.session_state.sleep_active=False; st.warning("😴 Sleep timer ended. Time to rest!"); st.rerun()
                else:
                    m,s = divmod(remaining,60)
                    total_dur = st.session_state.get("sleep_total_duration", sleep_mins*60)
                    st.progress(max(0.0,min(1.0,1-remaining/total_dur)))
                    st.markdown(f"<p style='text-align:center;color:#aaa;font-size:1.5rem;'>😴 Stopping in {m:02d}:{s:02d}</p>",unsafe_allow_html=True)
