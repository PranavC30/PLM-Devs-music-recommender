...
def load_stats(username):
    ...
    if os.path.exists(f):
        with open(f) as fp:
            return json.load(fp)
    return {
        "total_sessions": 0, "total_likes": 0, "total_skips": 0,
        "streak": 0, "last_active_date": None,
        "moods_tried": [], "genres_tried": [], "mood_counts": {},
        "earned_badges": [], "xp": 0,
    }
...
def update_stats(username, mood, genre, feedback):
    ...
    try:
        ...
        update_leaderboard(username, stats["xp"], get_level(stats["xp"]))
    except Exception:
        pass
    return stats, newly_earned

def get_level(xp):
    for threshold, name in [(1000,"🏆 Legend"),(600,"🌟 Audiophile"),
                             (300,"🎤 Enthusiast"),(150,"🎵 Music Fan"),
                             (50,"🎧 Listener"),(0,"🌱 Newbie")]:
        if xp >= threshold:
            return name
    return "🌱 Newbie"
