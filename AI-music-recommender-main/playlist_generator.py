...
# Average song duration assumptions (minutes) per energy level
...
def generate_playlist(
    ...
    genre: str = None,
    energy: str = None,
    avoid_songs: list = None,
) -> list:
    """
    Build a playlist of songs that fills approximately `duration_minutes`.

    Returns a list of song dicts with an added 'est_duration_min' key.
    """
    df = _load_df()
    ...
    pool = df.copy()
    if mood:
        pool = pool[pool["Mood"] == mood]
    if language:
        pool = pool[pool["Language"].str.lower() == language.lower()]
    if genre:
        pool = pool[pool["Genre"] == genre]
    if energy:
        pool = pool[pool["Energy"] == energy]

    # Fallback: relax filters progressively if pool too small
    if len(pool) < 3:
    ...
    # Build playlist to fill duration
    pool = pool.sample(frac=1).reset_index(drop=True)  # shuffle
    playlist = []
    total_mins = 0.0
